...
export default function Analytics() {
    ...
    const [motivationalMessage, setMotivationalMessage] = useState<
        string | null
    >(null);
    const [motivationalImage, setMotivationalImage] = useState<string | null>(
        null
    );
    ...
    const [editingMetric, setEditingMetric] = useState<CustomMetric | null>(
        null
    );
    ...
    const fetchAll = useCallback(async () => {
        ...
        try {
          ...
          console.log("moodsRes.error", moodsRes.error);
            ...
            if (!metricsRes.error)
                setMetrics((metricsRes.data as CustomMetric[]) ?? []);
            if (!tasksRes.error)
    ...
    moodEntries.forEach(e => {
        moodMap[e.date.split("T")[0]] = e.mood;
    });
    ...
    const monthlyData = Array.from({ length: today }, (_, i) => {
        ...
        return {
            day: d,
                ...
                t => t.status === "done" && t.created_at?.startsWith(prefix)
            ).length,
        };
    });
    ...
    return (
        <div>
                        ...
                        {todayMood && (
                            <p className="text-center text-sm text-muted-foreground mt-3">
                                ...
                                <span className="font-medium">
                                    {MOOD_EMOJIS[todayMood.mood - 1]}{" "}
                                    {MOOD_LABELS[todayMood.mood - 1]}
                                </span>
                            </p>
                        )}
                    </CardContent>
                ...
                <Card className="bg-card border shadow-sm">
                    <CardContent className="p-5">
                        <h3 className="text-lg font-semibold mb-1 text-foreground">
                            Орындалған тапсырмалар — айлық
                        </h3>
                        <p className="text-sm text-muted-foreground mb-4">
                            Орындалды: {doneTasks} / {totalTasks} (
                            {completionPct}%)
                        </p>
                        {loading ? (
                            <Skeleton className="h-56 w-full rounded-lg" />
                ...
                {loading ? (
                    ...
                    <Card className="bg-card border border-dashed shadow-sm">
                        <CardContent className="p-8 flex flex-col items-center gap-3 text-center">
                            ...
                            <p className="text-muted-foreground text-sm">
                                Сіздің өз метрикаларыңыз жоқ.
                                <br />
                                «Метрика қосу» батырмасын басып, бірінші
                                метриканы жасаңыз.
                            </p>
                            <Button
                                size="sm"
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        {metrics.map(m => (
                            <Card
                                key={m.id}
                                className="bg-card border shadow-sm group relative overflow-hidden">
                                <CardContent className="p-4">
                                    <div
                                        className={`w-8 h-1 rounded-full ${m.color} mb-3`}
                                    />
                ...
                <DialogContent className="sm:max-w-md">
                    ...
                    <div className="space-y-3 py-2">
                        ...
                        <div className="pt-2">
                            ...
                            <div className="rounded-xl border bg-card p-4 inline-block min-w-[140px]">
                                <div
                                    className={`w-8 h-1 rounded-full ${metricForm.color} mb-2`}
                                />
                    ...
                    <DialogFooter>
                        ...
                        <Button
                            ...
                            disabled={!metricForm.name.trim() || savingMetric}>
                            {savingMetric
                                ? "Сақталуда..."
                                : editingMetric
                                ? "Жаңарту"
            ...
            {/* Delete confirm */}
            <AlertDialog
                open={!!deletingMetricId}
                onOpenChange={open => !open && setDeletingMetricId(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Метриканы жою</AlertDialogTitle>
                        <AlertDialogDescription>
                            Бұл метриканы жойғыңыз келетінін растайсыз ба? Бұл
